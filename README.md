...
PNG растеризуется локальным CanvasKit. Перед экспортом `figgy` восстанавливает
точные глифы и контуры из FIG payload, данные переопределённых экземпляров,
унаследованную видимость вариантов и общие affine-transform матрицы. Сетки
редактора в результат не попадают.

Все сетевые источники шрифтов принудительно отключены. Шрифты ищутся только в
системных каталогах и в путях из `FIGGY_FONT_DIRS`; когда `.fig` содержит
производные глифы Figma, они имеют приоритет над повторным shaping локальным
шрифтом. Это исключает утечку названий шрифтов и текстовых подмножеств из
NDA-макета. PNG и особенно SVG сами содержат данные макета и должны
обрабатываться как такие же приватные артефакты, что и исходный `.fig`.

Для локальной пиксельной калибровки есть вспомогательная команда:

```bash
npm run compare:renders -- ./figma.png ./local.png ./diff.png
```

Она проверяет одинаковость размеров, считает exact-pixel ratio, MAE, RMSE,
PSNR и долю пикселей выше порогов Δ 4/8/16/32, а третьим аргументом может создать
красную heatmap. Эталоны MCP, локальные рендеры и diff приватного макета следует
хранить только внутри игнорируемого `for-tests/`.
